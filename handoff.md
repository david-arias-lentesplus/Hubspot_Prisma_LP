...
**Fix: PDF exportado se veía cortado (2026-07-09) — regresión del fix de "sidebar fijo":**

David reportó que el PDF exportado no mostraba toda la información y se veía cortado, y pidió además que el PDF reproduzca el mismo diseño/estructura que se ve en pantalla.

- **Causa raíz**: el ajuste de "Sidebar fijo al viewport" (ver más arriba) le dio a `<main>` una altura acotada al alto disponible de pantalla + `overflow-y: auto`, para que solo el contenido scrolleara sin arrastrar el sidebar. Ese mismo recorte, sin embargo, hizo que `html2canvas` — que captura el nodo tal cual está renderizado en el DOM — solo "viera" la porción visible en el momento del click (`clientHeight`, ej. 708px) y no el contenido completo (`scrollHeight`, ej. 1462px en la vista Resumen). Antes del fix de sidebar, `<main>` no estaba realmente acotado (el shell usaba `min-h-screen` sin `overflow-hidden`), así que por casualidad el PDF salía completo — el bug quedó latente hasta que se corrigió el layout.
- [x] **Fix en `src/utils/exportPdf.js`**: antes de llamar a `html2canvas`, `exportNodeToPdf()` guarda los estilos inline originales del nodo (`overflow`, `overflowY`, `height`, `maxHeight`), los fuerza temporalmente a `visible`/`auto`/`none` (patrón estándar para exportar contenedores con scroll interno), espera un frame (`requestAnimationFrame`) para que el navegador recalcule el layout con el alto real, captura con `html2canvas` (agregando también `height`/`windowHeight: node.scrollHeight` como refuerzo), y restaura los estilos originales en un bloque `finally` — se restauran siempre, incluso si la captura falla, para no dejar el layout roto en pantalla.
- [x] **Diseño/estructura consistente con la pantalla**: `DashboardLayout.jsx` ahora pasa `backgroundColor: isDark ? "#0B0B0F" : "#FFFFFF"` a `exportNodeToPdf()` según el tema activo (antes era siempre blanco fijo, lo que podía dejar zonas sin fondo propio inconsistentes si se exportaba en modo oscuro).
- [x] Verificado en navegador real: `document.querySelector('main')` en la vista "Resumen" mostraba `clientHeight: 708` vs `scrollHeight: 1462` (confirmando el bug); simulando la misma secuencia de override de estilos que usa `exportNodeToPdf()`, el nodo pasa a `offsetHeight: 1462` (contenido completo) durante la captura y vuelve a `clientHeight: 708` con `overflow-y: auto` intacto al restaurar — sin romper el scroll normal de la página. Click real en "Exportar informe" completó el ciclo (spinner → idle) sin errores en consola.
