...
- `exportPdf.js` (nuevo, Fase Enterprise): `exportNodeToPdf()` — html2canvas + jsPDF, ambas cargadas con `import()` dinámico. **Fix 2026-07-09**: capturaba solo la porción visible de `<main>` (bug introducido por el fix de "sidebar fijo", que le dio a `<main>` altura acotada + scroll interno) — ahora fuerza temporalmente `overflow: visible`/`height: auto` antes de capturar, así el PDF incluye todo el contenido, no solo lo visible en pantalla en el momento del click. El fondo del PDF también sigue el tema activo (claro/oscuro) en vez de ser siempre blanco
